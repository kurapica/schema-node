 ...
 */
export function prepareRuleSchema(node: SchemaNode<ISchemaNodeConfig>, parent?: SchemaNode<ISchemaNodeConfig>): ISchemaNodeRuleSchema {
    parent = parent || node.parent
    const ruleSchema = parent
        ? (
            // parent schema build first
...
export function activeRuleSchema(node: SchemaNode<ISchemaNodeConfig>, deep?: boolean, isinit?: boolean) {
    if (node.schemaType == SchemaType.Array) {
        ...
        (node as unknown as StructSchemaNode).fields?.forEach(f => activeRuleSchema(f, deep, isinit))
    }
 ...
 */
function buildRuleSchema(schemaInfo: ISchemaInfo): ISchemaNodeRuleSchema {
    const ruleSchema: ISchemaNodeRuleSchema = { type: schemaInfo.name }
    ...
    if (schemaInfo.type == SchemaType.Array) {
        ...
        if (elementInfo?.type === SchemaType.Struct && schemaInfo.array?.relations?.length) {
            for (let i = 0; i < schemaInfo.array.relations.length; i++) {
                registerRelation(eleRuleSchema, schemaInfo.array.relations[i], elementInfo.struct!)
            }
        }
